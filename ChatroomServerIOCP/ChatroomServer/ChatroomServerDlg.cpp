...
void CChatroomServerDlg::OnBnClickedButton1()
{
	...
	if (strStartStatus == _T("Start"))
	{
		...
		while (g_bRestart) {
			...
			if (g_hIOCP == NULL) {
				LogError(_T("CreateIoCompletionPort() failed to create I/O completion port"));
				goto FINISH;
			...
			for (DWORD dwCPU = 0; dwCPU < dwThreadCount; dwCPU++) {
				...
				if (hThread == NULL) {
					LogError(_T("CreateThread() failed to create worker thread"));
					goto FINISH;
				}
			...
			if (WAIT_OBJECT_0 != WaitForMultipleObjects(dwThreadCount, g_ThreadHandles, TRUE, 1000))
				LogError(_T("WaitForMultipleObjects() failed"));
			else
				for (DWORD i = 0; i < dwThreadCount; i++) {
